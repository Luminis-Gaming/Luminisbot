...
class LuminisbotCompanion:
    ...
    def write_savedvariables(self, events_data):
        ...
        try:
            ...
            # Read existing file to preserve LuminisbotCommands
            commands_lua = ""
            if path.exists():
                try:
                    with open(path, 'r', encoding='utf-8') as f:
                    ...
                    # Find LuminisbotCommands table (handle nested braces)
                    start_pos = existing_content.find('LuminisbotCommands = {')
                    if start_pos != -1:
                        # Find matching closing brace
                        brace_count = 0
                        pos = start_pos + len('LuminisbotCommands = ')
                        for i in range(pos, len(existing_content)):
                            if existing_content[i] == '{':
                                brace_count += 1
                            elif existing_content[i] == '}':
                                brace_count -= 1
                                if brace_count == 0:
                                    commands_lua = "\n" + existing_content[start_pos:i+1]
                                    break
                except Exception as e:
                    print(f"[WRITE] Warning: Could not preserve LuminisbotCommands: {e}")
...
"""
            ...
            # Read existing file to preserve pendingCommands
            pending_commands_lua = "{ }"
            last_command_id = 0
            ...
            if runtime_path.exists():
                try:
                    ...
                    import re
                    # Extract pendingCommands table (need to handle nested braces)
                    # Look for pendingCommands = { ... } including nested tables
                    pending_start = existing_content.find('pendingCommands = {')
                    if pending_start != -1:
                        # Find the matching closing brace
                        start = pending_start + len('pendingCommands = ')
                        brace_count = 0
                        end = start
                        for i in range(start, len(existing_content)):
                            if existing_content[i] == '{':
                                brace_count += 1
                            elif existing_content[i] == '}':
                                brace_count -= 1
                        ...
                        if end > start:
                            pending_commands_lua = existing_content[start:end]
                    
                    # Extract lastCommandId if it exists
                    id_match = re.search(r'lastCommandId\s*=\s*(\d+)', existing_content)
                    if id_match:
                        last_command_id = int(id_match.group(1))
                except Exception as e:
                    print(f"[WRITE] Warning: Could not preserve pending commands: {e}")
...
LuminisbotCompanionData = {{
    ...
    companionRunning = true,
    companionHeartbeat = {timestamp},
    pendingCommands = {pending_commands_lua},
...
"""
    ...
    def get_commands_path(self):
        ...
        # Commands are in the same file as events - use the same path
        return self.get_savedvariables_path()
    
    def read_commands(self):
        """Read and parse pending commands from LuminisbotCommands SavedVariable"""
        commands_path = self.get_commands_path()
        if not commands_path or not commands_path.exists():
            print(f"[CMD] Commands file not found: {commands_path}")
            return []
        
        try:
            print(f"[CMD] Reading commands from: {commands_path}")
            with open(commands_path, 'r', encoding='utf-8') as f:
                content = f.read()
            
            # Look for LuminisbotCommands.queue table
            import re
            
            # Find LuminisbotCommands table first
            commands_start = content.find('LuminisbotCommands = {')
            if commands_start == -1:
                print(f"[CMD] No LuminisbotCommands found in file")
                return []
            
            # Find the queue array within it (can be either queue = or ["queue"] =)
            queue_start = content.find('["queue"] = {', commands_start)
            search_string = '["queue"] = '
            if queue_start == -1:
                queue_start = content.find('queue = {', commands_start)
                search_string = 'queue = '
            ...
            if queue_start == -1:
                print(f"[CMD] No queue found in LuminisbotCommands")
                return []
            
            # Find the closing brace for the queue array (handle nested braces)
            brace_count = 0
            pos = queue_start + len(search_string)
            for i in range(pos, len(content)):
                if content[i] == '{':
                    ...
                    brace_count -= 1
                    if brace_count == 0:
                        queue_content = content[pos+1:i]
                        print(f"[CMD] Found queue with {len(queue_content)} characters")
                        break
            else:
                print(f"[CMD] Could not find end of queue array")
                return []
            
            # Parse each command entry
            commands = []
            # Find command tables - need to handle nested braces
            i = 0
            while i < len(queue_content):
                if queue_content[i] == '{':
                    # Found start of command
                    brace_count = 1
                    start = i
                    i += 1
                    while i < len(queue_content) and brace_count > 0:
                        if queue_content[i] == '{':
                            brace_count += 1
                        elif queue_content[i] == '}':
                            brace_count -= 1
                        i += 1
                    
                    # Extract the command block
                    block = queue_content[start:i]
                    
                    try:
                        cmd = {}
                        
                        # Extract fields (handle both ["key"] and key formats)
                        ...
                        event_id_match = re.search(r'\["?eventId"?\]\s*=\s*(\d+)', block)
                        if event_id_match:
                            cmd['eventId'] = int(event_id_match.group(1))
                        
                        # Extract data table (handle nested braces)
                        ...
                        if data_start != -1:
                            # Find matching closing brace
                            data_brace_count = 0
                            data_pos = block.find('{', data_start + 1)
                            for j in range(data_pos, len(block)):
                                if block[j] == '{':
                                    data_brace_count += 1
                                elif block[j] == '}':
                                    data_brace_count -= 1
                                    if data_brace_count == 0:
                                        data_content = block[data_pos+1:j]
                                        break
                            
                            cmd['data'] = {}
                            
                            char_match = re.search(r'\["?character"?\]\s*=\s*"([^"]*)"', data_content)
                            if char_match:
                                cmd['data']['character'] = char_match.group(1)
                            
                            realm_match = re.search(r'\["?realm"?\]\s*=\s*"([^"]*)"', data_content)
                            if realm_match:
                                cmd['data']['realm'] = realm_match.group(1)
                            
                            status_match = re.search(r'\["?status"?\]\s*=\s*"([^"]+)"', data_content)
                            if status_match:
                                cmd['data']['status'] = status_match.group(1)
                        
                        if 'id' in cmd and 'type' in cmd:
                            commands.append(cmd)
                            print(f"[CMD] Parsed command {cmd['id']}: {cmd['type']}")
                        else:
                            print(f"[CMD] Skipping incomplete command block")
                            
                    except Exception as e:
                        print(f"[CMD] Warning: Failed to parse command block: {e}")
                        continue
                else:
                    i += 1
    ...
    def clear_commands(self):
        """Clear the command queue in LuminisbotCommands SavedVariable"""
        commands_path = self.get_commands_path()
        if not commands_path or not commands_path.exists():
            return
        ...
        try:
            ...
            # Find LuminisbotCommands table and replace just the queue
            commands_start = content.find('LuminisbotCommands = {')
            if commands_start == -1:
                print(f"[CMD] No LuminisbotCommands found to clear")
                return
            
            # Find the queue array
            queue_start = content.find('["queue"] = {', commands_start)
            if queue_start == -1:
                queue_start = content.find('queue = {', commands_start)
            
            if queue_start == -1:
                print(f"[CMD] No queue found to clear")
                return
            
            # Find the end of the queue array
            search_string = '["queue"] = ' if '["queue"]' in content[commands_start:queue_start+20] else 'queue = '
            brace_count = 0
            pos = queue_start + len(search_string)
            queue_end = pos
            ...
            # Replace the queue content with empty
            new_content = content[:pos] + '{ }' + content[queue_end:]
            
            # Also reset lastCommandId to 0
            import re
            new_content = re.sub(
                r'(\["?lastCommandId"?\]\s*=\s*)\d+',
                r'\g<1>0',
                new_content
            )
            ...
            print(f"[CMD] Cleared command queue")
        except Exception as e:
    ...
    def process_command(self, command):
        ...
        elif cmd_type == "signup":
            character = data.get('character')
            realm = data.get('realm')
            
            if not character:
                print(f"[CMD] ✗ Missing character name")
                return False
            
            # Call API to sign up for event
            try:
                headers = {
                    'X-API-Key': self.api_key,
                    'Content-Type': 'application/json'
                ...
                payload = {
                    'character': character,
                    'realm': realm_slug,
                    'status': 'signed'  # New signups default to signed
                }
                ...
                if response.status_code == 200:
                    print(f"[CMD] ✓ Signed up {character}")
                    return True
                else:
                    print(f"[CMD] ✗ API error: {response.status_code}")
                    return False
    ...
    def sync_loop(self):
        ...
        while self.running:
            try:
                ...
                print(f"[CMD] Checking for pending commands...")
                ...
                if commands:
                    ...
                    print(f"[CMD] Processing {len(commands)} command(s) after deduplication")
                    for command in commands:
                        self.process_command(command)
                    # Clear commands after processing
                    self.clear_commands()
                else:
                    print(f"[CMD] No pending commands found")
